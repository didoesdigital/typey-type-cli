...
const getMatchingPrefix = (
  ...
  while (i < prefixes.length && !prefixFound) {
    ...
    if (wordOrPhraseMaterial.startsWith(prefix)) {
      prefixFound = true;
      return prefixEntry;
    }
...
const splitOnPrefix: SplitterFunction = (
  ...
  const prefixEntry = getMatchingPrefix(
    wordOrPhraseMaterial,
    affixList.prefixes,
  );
  if (prefixEntry === null) {
